	...
	<div class="content-wrap">
		...
		<div class="table-header">
			<div class="listInfo">
				...
				<div class="totalNum">
					<font color="#BDBDBD">전체</font> <span><font color="red">${impCount}</font></span>
				</div>
			...
			<table>
				...
				<c:forEach var="memoImp" items="${memoImp}"> 
					...
					<tr>
						<td>
							<div class="memoNote">
								${memoImp.memo_content}
							</div>
						</td>
					</tr>
					<tr>	
						<td> ${memoImp.getCate_title()} | 등록:${sdf.format(memoImp.getMemo_enroll())} | 수정:${sdf.format(memoImp.getMemo_modi())}</td>
					</tr>
					</c:forEach>
					</c:if>
			</table>
	...
	<div class="page-wrap">
		<table>
			...
			<c:if test="${startPage > 10}">
				<a href="memoImp.jp?pageNum=${startPage-10}">[이전]</a>
			</c:if>
		
			<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
				<a href="memoImp.jp?pageNum=${i}">${i}&nbsp;</a>
			</c:forEach>
		
			<c:if test="${endPage < pageCount}">
				<a href="memoImp.jp?pageNum=${startPage+10}">[다음]</a>
			</c:if>
