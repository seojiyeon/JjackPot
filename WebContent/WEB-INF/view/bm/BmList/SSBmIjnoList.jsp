...
<jsp:include page="bm_sidebar.jsp" flush="false" />
	<div id="main-contents" style="   font-family: serif;">
		...
		<tbody>
			...
			<c:forEach var="bmdto"  items="${SSBmIjnoList}">
				<tr>
                        <th style="width: 40px;"><input id="checkAll" name="" onclick="selectAllTodo()" type="checkbox" value="" title="checkAll"></th>
				...
				<table>
					<c:if test="${count > 0}">
						...
						<c:if test="${startPage > 10}">
							<a href="SSBmIjnoList.jp?pageNum=${startPage-10}" >[이전]</a>
						</c:if>
			
						<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
							<a href="SSBmIjnoList.jp?pageNum=${i}">${i}&nbsp;</a>
						</c:forEach>
		
						<c:if test="${endPage < pageCount}">
							<a href="SSBmIjnoList.jp?pageNum=${startPage+10}">[다음]</a>
						</c:if>
					</c:if>
				</table>
			</div>
